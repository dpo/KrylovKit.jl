...
_realview(v::AbstractVector{Complex{T}}) where {T} = view(reinterpret(T, v), 2*(1:length(v)) .- 1)
_imagview(v::AbstractVector{Complex{T}}) where {T} = view(reinterpret(T, v), 2*(1:length(v)))
